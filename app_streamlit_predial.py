...
def cargar_datos(uploaded_file):
    ...
    columnas_numericas = [
        'valor_impuesto_a_pagar', 'recaudo_predial',
        'avaluo_catastral', 'descuentos_impuesto_predial', 'area_construida'
    ]
    df[columnas_numericas] = df[columnas_numericas].apply(pd.to_numeric, errors='coerce').fillna(0)
    return df

@st.cache_data
def aplicar_filtros(df, sector, sector_urbano, vereda, uso, ph):
    if sector != 'Todos':
        df = df[df['sector_general'] == sector]
    if sector_urbano != 'Todos':
        df = df[df['sector_urbano'] == sector_urbano]
    if vereda != 'Todos':
        df = df[df['vereda'] == vereda]
    if uso != 'Todos':
        df = df[df['uso_predio'] == uso]
    if ph != 'Todos':
        df = df[df['ph'] == ph]
    return df
import pandas as pd
